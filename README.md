...
Delegations take canonical knobs the adapters translate per peer: `--effort low…max` (reasoning depth, default `high`) and `--model fast|standard|max` (capability tier). `--model` has **no default** — omitted, the bridge sends no model flag and the peer CLI's own configured model applies; unknown values pass through verbatim, so new model names work before any map is updated.
