...
A task is normally an argv, but `--task-file <path>` (or `-` for stdin) reads it from a file — use that for long briefs and for anything containing `$`, backticks or quotes, which a shell would otherwise eat. Adapters can declare an `artifacts_dir` (agy writes generated images into a private per-session directory); the bridge lists what a run left there once it finishes, so peer output outside the repo isn't invisible.
